...
const FirstAiredDate = ({ firstAirDate }: { firstAirDate: Date }) => {
    if (isInPast(firstAirDate)) return <p className="font-medium text-lg">First aired on {moment(firstAirDate).format("LL")}</p>;
...
const LastAiredDate = ({ lastAirDate }: { lastAirDate: Date }) => {
    if (isInPast(lastAirDate)) return <p className="font-medium text-lg">Last aired on {moment(lastAirDate).format("LL")}</p>
