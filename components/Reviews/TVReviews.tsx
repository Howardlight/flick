...
export const TVReviews = ({ tvID, className }: { tvID: number, className?: string }) => (
    <Reviews className={`${className}`} >
        <TVReviewsContent tvID={tvID} />
    </Reviews>
)
 ...
 * @returns
 */
export const TVReviewsContent = ({ tvID }: { tvID: number; }) => {
    ...
    const { data, error }: SWRResponse<ReviewResponse, Error> = useSWR(`/api/TV/getTVReviews/${tvID}/${page}`, fetcher);
            ...
            <Default>
                <Fragment>
                    {data!.results.map((tv, index) => {
                        return (
                            <Fragment key={`${tv.id}-${index}`}>
                                <div className="flex flex-row gap-4 mt-6 mb-6" key={`${tv.id}-${index}`}>
                                    <div className="w-auto h-auto mt-2">
                                        <ImageWithFallback
                                            alt={`${tv.author}'s Avatar`}
                                            src={tv.author_details.avatar_path ? tv.author_details.avatar_path : Placeholder.src}
                                    ...
                                    <div className="w-5/6 border p-2 rounded-md border-neutral-400">
                                        <div className="flex flex-row justify-between h-16">
                                            <div className="flex flex-col">
                                                <p>{tv.author}</p>
                                                <p className="text-base font-medium text-neutral-400">{moment(tv.created_at).format("LL")}</p>
                                            </div>
                                            {tv.author_details.rating ?
                                                <div className="flex flex-row items-center justify-center mr-4 text-2xl font-semibold gap-2">
                                                    <p className="text-lg">{tv.author_details.rating} / 10</p>
                                                    <Image
                                                        src={Star.src}
                                                        alt={"Star"}
                                                        width={32}
                                        ...
                                        <Comment className="" text={tv.content} />
                                    </div>
                                </div>
