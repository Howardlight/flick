...
import { TVParamsPage } from "../../paramsType";

export async function GET(request: NextResponse, { params }: { params: TVParamsPage }) {
    const TVID = params.TVID;
    const page = params.Page;
    let data;

    const req = await fetch(`https://api.themoviedb.org/3/tv/${TVID}/reviews?api_key=${process.env.TMDB_API_KEY}&language=en-US&page=${page}`);
    data = await req.json();
